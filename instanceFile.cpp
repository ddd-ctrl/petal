...
{ metaData_ = &metadata_;
  ...
  metadata_.filename = dataFileName;
  line = 0;
  count_ = 0;

  f = fopen(dataFileName, "r");

  if (f == NULL) error("Cannot open input file %s", dataFileName);
}
...
bool InstanceFile::advance(instance &inst) {
  if (metadata_.inputFormat_ == ioMetadata::PETAL_FORMAT) {
    int c = getc(f);
    ...
    while (c != '\n' && c!= '\r' && c != EOF) {
      ...
      switch (metadata_.attTypes[att]) {
        ...
        case CLASS:
          ...
          break;
        case NUMERIC:
          setNumVal(inst, metadata_.internalAtt[att], metadata_.readNum(f, metadata_.internalAtt[att], c, line));
          break;
      }

      // skips spaces after a value up to a comma, the comma (if present) and then any more spaces up to the next non-comma
    ...
    while (c != '\n' && c != EOF) {
      ...
      setNumVal(inst, att, metadata_.readNum(f, att, c, line));
...
bool InstanceFile::isAtEnd() const {
  int c = fgetc(f);
...
const char* InstanceFile::ioMetadata::getName() const {
  return filename;
}

///< true iff name comparisons are case sensitive
bool InstanceFile::ioMetadata::areNamesCaseSensitive() const {
  return caseSensitive;
}

/// return the name for a class
const char* InstanceFile::ioMetadata::getClassName(const CatValue y) const {
...
unsigned int InstanceFile::ioMetadata::getNoValues(const CategoricalAttribute att) const {
  assert(att < attValNames.size());
  return attValNames[att].size();
}

const char* InstanceFile::ioMetadata::getCatAttName(const CategoricalAttribute att) const {
  return attNames[cat2io[att]];
}

const char* InstanceFile::ioMetadata::getCatAttValName(const CategoricalAttribute att, const CatValue val) const {
...
const char* InstanceFile::ioMetadata::getNumAttName(const NumericAttribute att) const {
  return attNames[num2io[att]];
}

/// return the precision to which values of a numeric attribute should be output
unsigned int InstanceFile::ioMetadata::getPrecision(const NumericAttribute att) const {
  return precision[att];
}

/// return whether a categorical attribute contains missing values
bool InstanceFile::ioMetadata::hasCatMissing(const CategoricalAttribute att) const {
  return missingVals_[cat2io[att]];
}

/// return whether a numeric attribute contains missing values
bool InstanceFile::ioMetadata::hasNumMissing(const NumericAttribute att) const {
  return missingVals_[num2io[att]];
}
...
void InstanceFile::ioMetadata::setAllAttsMissing(){
  missingVals_.assign(noOfAttributes(),1);
}
...
void InstanceFile::ioMetadata::readName(FILEtype *f, char *buffer, const char terminator, int &c) const {
  ...
  while (true) {
    ...
    if (c >= ' ' && i < MAX_NAME_LENGTH) {  // °´ maxNameLength ½Ø¶ÏÃû³Æ£¬¶ªÆú·Ç´òÓ¡×Ö·û
...
void InstanceFile::ioMetadata::parse(const char *fn) {
  ...
  while (c != EOF) {
    ...
    if (streq(buffer, "numeric")) {
      ...
      if(c == ','){
        ...
        if (c != '?') {
          error("Expected '?' but encountered '%c' in numeric declaration on line %"  LCFMT, c, line);
        }
        missing = 1;
        c = getc(f);
      }
      for (int i = startIndex; i <= endIndex; i++) {
        ...
        num2io.push_back(noAttributes++);
        precision.push_back(0);
        missingVals_.push_back(missing);
      }
    }
    else {
  ...
  assert(missingVals_.size() == noAttributes);
}
...
NumValue InstanceFile::ioMetadata::readNum(FILEtype *f, const NumericAttribute att, int &c, const LineCount line) {
  assert(noOfAttributes()>0); // check that the metadata has been parsed before it is used

  if (c == '?') {
    // missing value

    if (!hasNumMissing(att)) error("Encountered a missing value for %s on line %" LCFMT, getNumAttName(att), line);
...
bool instance::get(FILEtype *f, LineCount &line) {
  if (metadata_.inputFormat == ifPetal) {
    int c = getc(f);
