...
static LearnerRegistrar registrar("rfdt", constructor<RFDTree>);

char*const* nullPtr = NULL;

RFDTree::RFDTree(char*const*& argv, char*const* end)
: DTree(nullPtr, nullPtr), trainingParams_(10,1.0,10)
{ name_ = "RFDTree";
...
void RFDTree::train(InstanceStream &is) {
  ...
  for (CatValue y = 0; y < noOfClasses; ++y) {
    priors.push_back(static_cast<NumValue>(1.0/noOfClasses));
  }
