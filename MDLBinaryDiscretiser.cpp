...
void MDLBinaryDiscretiser::FindCutPoints(std::vector<SubInstance> data,
...
double MDLBinaryDiscretiser::compEntropy(unsigned int noInst,
	...
	for (unsigned int j = 0; j < noOfClasses_; j++) {
		if (classCount[j] > 0) {
			pr_each_class = static_cast<double>( classCount[j]) / noInst;
...
void MDLBinaryDiscretiser::discretise(std::vector<NumValue> &vals,
		const std::vector<CatValue> &classes, unsigned int noOfClasses, std::vector<NumValue> &cuts) {

	if (vals.size() != classes.size()) {
		error("the sizes of vals and classes are not identical.\n");
	}
	...
	if (verbosity >= 3) {
		...
		for (unsigned int i = 0; i < classAll.size(); i++) {
			printf("%d\t%d\n", i, classAll[i]);
		}
	}
