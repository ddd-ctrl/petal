...
FeatingLearner::FeatingLearner(char*const*& argv, char*const* end) : learnerName_(NULL), discretiser_("mdl2", argBegin, argEnd)
...
void FeatingLearner::train(InstanceStream &is) {
  ...
  if (aStream->getNoNumAtts() > 0) {
    ...
    for (NumericAttribute a = 0; a < aStream->getNoNumAtts(); ++a) {
      ...
      //if the maxVal does not change in the loop, then the other valude of attribute a can not reach
      //It is better to use metaData.cuts.size()
      while (v <= maxVal) {
        thisStream.setSourceWithoutLoading(*aStream);  // clear the stream
