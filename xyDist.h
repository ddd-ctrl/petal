...
public:
  ...
  void reset(InstanceStream *is, unsigned int noOrigCatAtts); ///< initialise only for the originally discrete attributes, used by gnb for example.
 ...
 // p(Y=y|a=v) using M-estimate
  inline double pYAtt(CategoricalAttribute a, CatValue v, CatValue y) {
    return mEstimate(counts_[a][v*noOfClasses_+y], getCount(a,v), noOfClasses_);
  }
