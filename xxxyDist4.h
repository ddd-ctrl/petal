...
class constXXYSubDist4 {
public:
	constXXYSubDist4(std::vector<std::vector<InstanceCount> >* subDist, const unsigned int noOfClasses) : subDist_(subDist), noOfClasses_(noOfClasses){}

  inline std::vector<InstanceCount>* getXYSubDist(const CategoricalAttribute x, const CatValue v) const { return &subDist_[x][v*x]; }
...
class XXYSubDist4 {
public:
  XXYSubDist4(std::vector<std::vector<InstanceCount> >* subDist, const unsigned int noOfClasses) : subDist_(subDist), noOfClasses_(noOfClasses) {}
...
public:
	xxxyDist4();
	xxxyDist4(InstanceStream& stream);
	~xxxyDist4(void);
		...
		return (*constRef(x1, v1, x2, v2, x3, v3, y)
				+ M
						/ (getNoValues(x1) * getNoValues(x2) * getNoValues(x3)
								* noClasses_)) / (xxyCounts_.xyCounts.count + M);
	}
...
// p(x1=v1|Y=y, x2=v2, x3=v3) using M-estimate
	inline double p(CategoricalAttribute x1, CatValue v1,
		...
		return (*constRef(x1, v1, x2, v2, x3, v3, y) + M / getNoValues(x1))
				/ (xxyCounts_.getCount(order_[x2], v2, order_[x3], v3, y) + M);
	}
...
//	}
	...
	inline unsigned int getNoClasses() const {
	  ...
	  inline std::vector<InstanceCount >* getXYSubDist(CategoricalAttribute x1, CatValue v1,CategoricalAttribute x2, CatValue v2) {
	    return &count_[x1][v1*x1+x2][v2*noSelectedCatAtts_];
	  }
...
private:
	...
	inline InstanceCount *ref(CategoricalAttribute x1, CatValue v1,
		...
		return &count_[x1][v1 * x1 + x2][v2 * x2 + x3][v3 * noClasses_ + y];
	}
...
private:
	...
	std::vector<std::vector<std::vector<std::vector<InstanceCount> > > > count_;
