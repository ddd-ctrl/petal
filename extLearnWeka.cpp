...
//                fprintf(f, "%d,", inst.getCatVal(a));
            fprintf(f, "%d,", inst.getCatVal(a)); //Missing values have already been added as a new value
        } 
...
//				} else {
//					(last) ? fprintf(f, "%d", i) : fprintf(f, "%d,", i);
//				}
                                (last) ? fprintf(f, "%d", i) : fprintf(f, "%d,", i);//Missing values have already been added as a new value
			}
			fprintf(f, "}\n");
        }
        for (NumericAttribute a = 0; a < instanceStream->getNoNumAtts(); a++) {
...
void extLearnWeka::classify(InstanceStream *instanceStream, int fold) {
	...
	std::string wekaCommand;
        std::string firstArg = args[j];
        if((argcnt!=0) && ((firstArg.find("Xmx", 0) != std::string::npos))){
          wekaCommand = "java "+firstArg+" -cp ";   
          j++;
        }
        else
          wekaCommand = "java -cp ";
	...
	wekaCommand += ' '; wekaCommand += "-T "; wekaCommand += tmpTest.c_str();
        ...
        // Delete tmpOutput if it already exists
        if(fold==0)
...
void extLearnWeka::printResults(InstanceStream *instanceStream, unsigned int exp){
            ...
            getline(wekaOutput,item);
              if((offset = item.find("Error on test", 0)) != std::string::npos) {
                ...
                while((offset = item.find("Incorrectly", 0)) == std::string::npos)
                  ...
                  //As the corresponding  Weka output file is not found,
                  // it has been changed to Weka RMSE
                //RMSE (petal)
                while((offset = item.find("(Petal)", 0)) == std::string::npos)
                  ...
                  // Weka RMSE
//                  while((offset = item.find("Root", 0)) == std::string::npos)
//                      std::getline(wekaOutput,item,' ');
//                    std::getline(wekaOutput,item,' ');//mean
//                    std::getline(wekaOutput,item,' '); //square
//                    std::getline(wekaOutput,item,' '); //error
//                    std::getline(wekaOutput,item,' ');
//                    while(streq(item.c_str(),""))
//                      std::getline(wekaOutput,item,' ');  //absolute value
//                    item = item.substr(0,6);
